...
# Initial state is moving towards the goal
state = MOVING_TO_GOAL

RETURNING_TO_CLOSEST_POINT = 3

# Additional Variables
start_circumnavigating_point = None
min_distance_to_goal_while_circumnavigating = float('inf')
...
while robot.step(time_step) != -1:
    ...
    if state == MOVING_TO_GOAL:
        if hit_obstacle():
            state = CIRCUMNAVIGATING
            start_circumnavigating_point = (current_x, current_y)
            min_distance_to_goal_while_circumnavigating = distance
            follow_wall()
        else:
            towards_goal() 
    
    elif state == CIRCUMNAVIGATING:
        dist_to_start_circumnavigating = math.sqrt((current_x - start_circumnavigating_point[0])**2 + (current_y - start_circumnavigating_point[1])**2)
        if dist_to_start_circumnavigating < tolerance:
            state = RETURNING_TO_CLOSEST_POINT  # Transition to RETURNING_TO_CLOSEST_POINT instead of MOVING_TO_GOAL
            follow_wall()
            continue
        
        if distance < min_distance_to_goal_while_circumnavigating:
            min_distance_to_goal_while_circumnavigating = distance
        
        follow_wall()  # This line should come here. It ensures the robot always tries to follow the wall while circumnavigating.
    
    elif state == RETURNING_TO_CLOSEST_POINT:
        # Here, you should follow the wall until you reach the closest point
        current_distance_to_goal = math.sqrt((current_x - target_x)**2 + (current_y - target_y)**2)
        if current_distance_to_goal <= min_distance_to_goal_while_circumnavigating + tolerance:
            state = MOVING_TO_GOAL
            towards_goal()
            continue
